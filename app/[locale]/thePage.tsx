...
//   textsDataArray,
{
  ...
  const [pageListId, setPageListId] = useState(
    localStorage.getItem("pageListId") || "All",
  );
  ...
  const getSessionUser = async () => {
    const res = await axios.get(
      `/api/get-session-user?email=${session?.user?.email}&locale=${locale}`,
      {
        params: {
  ...
  return (
    ...
    >
      {locale === "ar" && (
        <Link href={`/en`} className="text-left w-full">
        ...
        {lists.map((list: List) => (
          ...
          >
            ...
            <ListEditDelete
              ...
              getLists={getLists}
              // email={user?.email} userId={user?.id}
              setPageListId={setPageListId}
              pageListIdQuery={pageListIdQuery}
            />
          </Badge>
        ))}
